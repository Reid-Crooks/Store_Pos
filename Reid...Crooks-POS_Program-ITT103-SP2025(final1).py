...
def store_header(text, option1, option2):
    ...
    print_line()
    print(f"# {header} #")
    print(f"# Date: {str(Constants.DATE).ljust(38)}Cart Items: {str(len(Cart.items)).rjust(3, '0')} | {format_currency(Cart.subtotal_price)} #")
    print_line()
    ...
    def print_list(self):
        ...
        for product_id, product, price, stock in self.items:
            if stock <= Constants.MIN_INVENTORY_AMT:
                ...
                low_stock =""
            print(f"{str(product_id).ljust(6)}\t\t{product.ljust(20)}\t\t\t{format_currency(price)}\t\t\t\t\t{int(stock)} {low_stock}")
    ...
    def print_list(self):
        ...
        for key, amt in self.items:
            for item_id, name, price, stock in inventory.items:
                if item_id == key:
                    print(f"{str(item_id).ljust(25)}{str(name).ljust(20)}{str(format_currency(price)).ljust(23)}{int(amt)}")
...
while True:
    if screen == 'F':
        ...
        if is_string(Cart.payment):
            ...
            print(f"Cannot process payment value of {format_currency(Cart.payment)} is less then total {format_currency(Cart.subtotal_price_tax)}")
        else:
            screen = 'V'
    elif screen == 'R':
        remove_from_cart()
